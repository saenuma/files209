...
func (cl *Client) ReadFile(groupName, fileName string) ([]byte, error) {
	...
	resp, err := httpCl.PostForm(fmt.Sprintf("%sread-file/%s/%s", cl.Addr, groupName, fileName), urlValues)
	if err != nil {
	...
	if resp.StatusCode == 200 {
		ret, err := base64.StdEncoding.DecodeString(string(body))
		if err != nil {
			return nil, ConnError{"base64 error\n" + err.Error()}
		}
		return ret, nil
	} else if resp.StatusCode == 400 {
